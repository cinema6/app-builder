...
var plugins = (function(plugins) {
    return {
        js: plugins.js.map(getPlugin),
        css: plugins.css.map(getPlugin)
    };
}(JSON.parse(process.argv[3])));
var parser = trumpet();

function getPlugin(plugin) {
    return require(resolve.sync(plugin, { basedir: config.baseDir }));
}
...
function pump(/*...args*/) {
    ...
    return args.reduce(function(from, to) {
        return from.pipe(to);
    });
}
...
parser.selectAll('link[rel="stylesheet"]', function getStylesheet(link) {
    ...
    html.append(pump(file, replaceStream(/url\(['"]?(.+?)['"]?\)/g, function(match, url) {
        ...
        var newURL = relative ?
            ('./' + ensurePosixPath(relativePath(config.baseDir, resolvePath(folder, url)))) :
            url;
